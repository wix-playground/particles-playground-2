(function(){"use strict";const X="random",b="DEV_EXAMPLE",k=`WIX 🤠
Particles!`,Y="#ffffff",R=["#ff0000","#00ff00","#0000ff"],B={fontFamily:"Arial",fontSize:64,italic:!1,weight:600,letterSpacing:0,lineHeight:1.2},F=`// This function will be called twice for each particle, because all particles reach the target in two frames.
...
}`,W=()=>Object.assign({},{[b]:{code:`${f}${U}`},DEV_TWO_FRAMES:{code:`${f}${F}`},bezier:{code:`${f}${N}`},pulseColorCycle:{code:`${f}${q}`},timeDistortion:{code:`${f}${H}`},elasticPlop:{code:`${f}${$}`}},...L.map(({name:e,comment:a,definition:c})=>({[e]:{code:`${f}return (particle, animationStartTime, currentTime, canvasDimensions, animationDuration) => {
