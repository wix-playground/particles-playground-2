(function(){"use strict";const Y="random",y="backInOut",R=`WIX 🤠
Particles!`,X="#ffffff",w=["#ff0000","#00ff00","#0000ff"],L={fontFamily:"Arial",fontSize:64,italic:!1,weight:600,letterSpacing:0,lineHeight:1.2},k=`// This function will be called twice for each particle, because all particles reach the target in two frames.
...
return (particle, animationStartTime, currentTime, canvasDimensions, animationDuration) => {
  ...
  t < 0.5
    ...
    */`},{name:"elasticIn",definition:` const elasticIn = (x) => {
    ...
    }`,comment:""}],W=`return (particle, animationStartTime, currentTime, canvasDimensions, animationDuration) => {
    const totalElapsedTime = currentTime - animationStartTime;
...
}`,z=()=>Object.assign({},{[y]:{code:`${T}${F}`},DEV_TWO_FRAMES:{code:`${T}${k}`},bezier:{code:`${T}${W}`},pulseColorCycle:{code:`${T}${$}`},timeDistortion:{code:`${T}${j}`},elasticPlop:{code:`${T}${Z}`}},...H.map(({name:e,comment:a,definition:r})=>({[e]:{code:`${T}return (particle, animationStartTime, currentTime, canvasDimensions, animationDuration) => {
...
};`}}))),b=(e,a)=>{const{width:o,height:n,data:c}=e,l=Math.ceil(o/a),s=Math.ceil(n/a),i=new Uint8Array(Math.ceil(o/a)*Math.ceil(n/a));let d=0;for(let h=0;h<n;h+=a)for(let u=0;u<o;u+=a){let g=!1;for(let m=0;m<a&&!g;m++)for(let p=0;p<a&&!g;p++){const v=u+p,M=h+m;if(v<o&&M<n){const nt=(M*o+v)*4;c[nt+3]>10&&(g=!0)}}i[d++]=g?1:0}return{validBlocks:i,blockWidth:l,blockHeight:s}},E=(e,a,r)=>e+r*(a-e),D=e=>{e=e.replace(/^#/,"");const a=parseInt(e,16),r=a>>16&255,o=a>>8&255,n=a&255;return{r,g:o,b:n}},Q=(e,a,r)=>"#"+((1<<24)+(e<<16)+(a<<8)+r).toString(16).slice(1),_=(e,a)=>{if(!(e!=null&&e.length))return"#ffffff";if(e.length===1)return e[0];const o=Math.max(0,Math.min(1,a))*(e.length-1),n=Math.floor(o);if(n===e.length-1)return e[e.length-1];const c=o-n,l=D(e[n]),s=D(e[n+1]),i=Math.round(E(l.r,s.r,c)),d=Math.round(E(l.g,s.g,c)),h=Math.round(E(l.b,s.b,c));return Q(i,d,h)},A=({dimensions:{width:e,height:a},emitterX:r=e/2,emitterY:o=a/2,emitterSize:n=100,emitterAngle:c=0})=>({top:()=>({x:Math.random()*e,y:0}),center:()=>({x:Math.round(e/2),y:Math.round(a/2)}),bottom:()=>({x:Math.random()*e,y:a}),random:()=>({x:Math.random()*e,y:Math.random()*a}),left:()=>({x:0,y:Math.random()*a}),right:()=>({x:e,y:Math.random()*a}),"top-left":()=>({x:Math.random()*(e/5),y:Math.random()*(a/5)}),"top-right":()=>({x:e,y:Math.random()*(a/5)}),"bottom-left":()=>({x:Math.random()*(e/5),y:a-Math.random()*(a/5)}),"bottom-right":()=>({x:e-Math.random()*(e/5),y:a-Math.random()*(a/5)}),canvasEdges:()=>{const s=Math.floor(Math.random()*4);return s===0?{x:Math.random()*e,y:0}:s===1?{x:Math.random()*e,y:a}:s===2?{x:0,y:Math.random()*a}:{x:e,y:Math.random()*a}},topLeft:()=>({x:0,y:0}),emitterPoint:()=>({x:r,y:o}),emitterCircle:()=>{const s=Math.random()*2*Math.PI,i=Math.random()*n;return{x:r+i*Math.cos(s),y:o+i*Math.sin(s)}},emitterSquare:()=>({x:r+(Math.random()-.5)*n,y:o+(Math.random()-.5)*n}),emitterHLine:()=>{const s=c*(Math.PI/180),i=(Math.random()-.5)*n;return{x:r+i*Math.cos(s),y:o+i*Math.sin(s)}},emitterVLine:()=>{const s=(c+90)*(Math.PI/180),i=(Math.random()-.5)*n;return{x:r+i*Math.cos(s),y:o+i*Math.sin(s)}},enterTopTextWidth:()=>({x:e*.2+Math.random()*e*.6,y:a*.3}),enterBottomTextWidth:()=>({x:e*.2+Math.random()*e*.6,y:a*.7}),enterLeftTextHeight:()=>({x:e*.2,y:a*.3+Math.random()*a*.4}),enterRightTextHeight:()=>({x:e*.8,y:a*.3+Math.random()*a*.4})});let S;const O={particleRadius:5,startPosition:Y,selectedMovementFunction:y,movementFunctionCode:z()[y].code,text:R,font:L,particleColors:w,animationDuration:3e3,particleSpread:3,startParticleOpacity:1,endParticleOpacity:1,startParticleSize:5,endParticleSize:1,delay:0,emitterX:U,emitterY:B,emitterSize:N,emitterAngle:q},t={workerParticles:[],imageBitmap:null,animationFrameId:0,frameCanvas:null,frameContext:null,mainCanvas:null,mainContext:null,validBlocks:null,blockHeight:0,blockWidth:0,appProps:O,revealProgress:0};let P;const V=async e=>{t.mainCanvas=e,t.mainContext=t.mainCanvas.getContext("bitmaprenderer"),t.frameCanvas=new OffscreenCanvas(t.mainCanvas.width,t.mainCanvas.height),t.frameContext=t.frameCanvas.getContext("2d",{willReadFrequently:!0})},K=e=>{const{imageBitmap:a,canvas:r,dimensions:o,appProps:n}=e;t.imageBitmap=a,Object.keys(n).length&&(t.appProps={...O,...n}),V(r),t.frameContext.drawImage(t.imageBitmap,0,0);const{validBlocks:c,blockHeight:l,blockWidth:s}=b(t.frameContext.getImageData(0,0,t.mainCanvas.width,t.mainCanvas.height),t.appProps.particleRadius);t.validBlocks=c,t.blockHeight=l,t.blockWidth=s,P=A({dimensions:o,emitterX:t.appProps.emitterX,emitterY:t.appProps.emitterY,emitterSize:t.appProps.emitterSize,emitterAngle:t.appProps.emitterAngle}),t.workerParticles=x({validBlocks:t.validBlocks,radius:t.appProps.particleRadius,blockHeight:t.blockHeight,blockWidth:t.blockWidth,startPosition:t.appProps.startPosition,delay:t.appProps.delay,animationDuration:t.appProps.animationDuration}),self.postMessage({type:I.INITIALIZED,data:t.appProps})},x=({validBlocks:e,radius:a,blockHeight:r,blockWidth:o,startPosition:n,delay:c,animationDuration:l})=>{const s=[];for(let i=0;i<r;i++)for(let d=0;d<o;d++){const h=i*o+d;if(e[h]){const u=d*a,g=i*a,{x:m,y:p}=P[n](),v=Math.random()*c,M=l-v;s.push({targetX:u,targetY:g,x:m,y:p,initialX:m,initialY:p,scale:1,opacity:1,color:X,revealProgress:0,revealThreshold:.97+Math.random()*.02,reachedTarget:!1,emittedBubbles:!1,delay:v,lifetime:M})}}return s},G=(e,a)=>{if(a>(e.revealThreshold||.99))return 1;if(a>.85&&Math.sqrt(Math.pow(e.x-e.targetX,2)+Math.pow(e.y-e.targetY,2))<=5){const n=(e.revealThreshold||.99)-.02,c=Math.max(0,(a-n)/.02);return Math.min(1,c)}return 0},J=e=>{const a=t.appProps.startParticleOpacity,r=t.appProps.endParticleOpacity;return a+(r-a)*e},tt=e=>{const a=t.appProps.startParticleSize,r=t.appProps.endParticleSize;return a+(r-a)*e},C=(e,a)=>{let r=!0;t.frameContext.clearRect(0,0,t.frameCanvas.width,t.frameCanvas.height);const o=a-e;t.revealProgress=Math.min(1,o/t.appProps.animationDuration),t.frameContext.globalAlpha=1,t.workerParticles.forEach(i=>{if(i.delay>a-e)return;const d=o-i.delay,h=Math.max(0,Math.min(1,d/i.lifetime));S(i,e,a,{width:t.mainCanvas.width,height:t.mainCanvas.height},t.appProps.animationDuration);const u=G(i,t.revealProgress),g=J(h),m=tt(h);if(u>0&&u<1){const p=t.appProps.particleRadius*(m||1);t.frameContext.globalAlpha=g*(1-u),t.frameContext.beginPath(),t.frameContext.arc(Math.floor(i.x)+p/2,Math.floor(i.y)+p/2,p/2,0,2*Math.PI),t.frameContext.fillStyle=t.appProps.particleColors.length?_(t.appProps.particleColors,t.revealProgress):i.color,t.frameContext.fill(),t.frameContext.globalAlpha=u*g,t.frameContext.drawImage(t.imageBitmap,i.targetX,i.targetY,m,m,Math.floor(i.x),Math.floor(i.y),m,m)}else if(u>=1)t.frameContext.globalAlpha=1,t.frameContext.drawImage(t.imageBitmap,i.targetX,i.targetY,t.appProps.particleRadius,t.appProps.particleRadius,Math.floor(i.x),Math.floor(i.y),t.appProps.particleRadius,t.appProps.particleRadius);else{const p=t.appProps.particleRadius*(m||1);t.frameContext.globalAlpha=g,t.frameContext.beginPath(),t.frameContext.arc(Math.floor(i.x)+p/2,Math.floor(i.y)+p/2,p/2,0,2*Math.PI),t.frameContext.fillStyle=t.appProps.particleColors.length?_(t.appProps.particleColors,t.revealProgress):i.color,t.frameContext.fill()}(i.x!==i.targetX||i.y!==i.targetY||t.revealProgress<.99)&&(r=!1)});const n=t.frameCanvas.transferToImageBitmap();t.mainContext.transferFromImageBitmap(n);const c=r&&t.revealProgress>=1,l=t.appProps.animationDuration;c&&o>=l?t.animationFrameId&&(cancelAnimationFrame(t.animationFrameId),t.frameContext.drawImage(t.imageBitmap,0,0),self.postMessage({type:I.ANIMATION_COMPLETE})):t.animationFrameId=requestAnimationFrame(i=>C(e,i))},et=()=>{t.animationFrameId&&cancelAnimationFrame(t.animationFrameId),S=new Function(t.appProps.movementFunctionCode)();const e=performance.now();t.revealProgress=0,C(e,e)},at=()=>{t.animationFrameId&&cancelAnimationFrame(t.animationFrameId),t.workerParticles=t.workerParticles.map(a=>{const r=P[t.appProps.startPosition](),o=Math.random()*t.appProps.delay,n=t.appProps.animationDuration-o;return{x:r.x,y:r.y,initialX:r.x,initialY:r.y,targetX:a.targetX,targetY:a.targetY,scale:1,opacity:1,color:a.color,revealProgress:0,revealThreshold:a.revealThreshold,delay:o,lifetime:n}}),t.frameContext.clearRect(0,0,t.frameCanvas.width,t.frameCanvas.height);const e=t.frameCanvas.transferToImageBitmap();t.mainContext.transferFromImageBitmap(e),t.animationFrameId&&cancelAnimationFrame(t.animationFrameId)},it=e=>{if(t.imageBitmap=e,t.frameCanvas&&t.mainCanvas){t.frameCanvas.width=t.imageBitmap.width,t.frameCanvas.height=t.imageBitmap.height,t.mainCanvas.width=t.imageBitmap.width,t.mainCanvas.height=t.imageBitmap.height,t.frameContext.drawImage(t.imageBitmap,0,0);const{validBlocks:a,blockHeight:r,blockWidth:o}=b(t.frameContext.getImageData(0,0,t.mainCanvas.width,t.mainCanvas.height),t.appProps.particleRadius);t.validBlocks=a,t.blockHeight=r,t.blockWidth=o,P=A({dimensions:{width:t.mainCanvas.width,height:t.mainCanvas.height},emitterX:t.appProps.emitterX,emitterY:t.appProps.emitterY,emitterSize:t.appProps.emitterSize,emitterAngle:t.appProps.emitterAngle}),t.frameContext.clearRect(0,0,t.frameCanvas.width,t.frameCanvas.height);const n=t.frameCanvas.transferToImageBitmap();t.mainContext.transferFromImageBitmap(n),t.workerParticles=x({validBlocks:t.validBlocks,radius:t.appProps.particleRadius,blockHeight:t.blockHeight,blockWidth:t.blockWidth,startPosition:t.appProps.startPosition,delay:t.appProps.delay,animationDuration:t.appProps.animationDuration})}},rt=e=>{const{appProps:a,shouldUpdateStartCoordinatesConfig:r,shouldRegenerateImageBlocks:o}=e;if(Object.assign(t.appProps,a),o){t.frameContext.drawImage(t.imageBitmap,0,0);const{validBlocks:n,blockHeight:c,blockWidth:l}=b(t.frameContext.getImageData(0,0,t.mainCanvas.width,t.mainCanvas.height),t.appProps.particleRadius);t.validBlocks=n,t.blockHeight=c,t.blockWidth=l}if(r&&(P=A({dimensions:{width:t.mainCanvas.width,height:t.mainCanvas.height},emitterX:t.appProps.emitterX,emitterY:t.appProps.emitterY,emitterSize:t.appProps.emitterSize,emitterAngle:t.appProps.emitterAngle})),t.workerParticles=x({validBlocks:t.validBlocks??new Uint8Array,radius:t.appProps.particleRadius,blockHeight:t.blockHeight,blockWidth:t.blockWidth,startPosition:t.appProps.startPosition,delay:t.appProps.delay,animationDuration:t.appProps.animationDuration}),self.postMessage({type:I.UPDATE_APP_PROPS,data:t.appProps}),t.animationFrameId){cancelAnimationFrame(t.animationFrameId);const n=performance.now();C(n,n)}};self.onmessage=e=>{const{payload:a,type:r}=e.data;switch(r){case f.INITIALIZE:{K(a);break}case f.PLAY:{et();break}case f.RESET:{at();break}case f.UPDATE_APP_PROPS:{rt(a);break}case f.UPDATE_BITMAP:{it(a);break}}}})();
