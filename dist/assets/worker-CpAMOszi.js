(function(){"use strict";const w="random",b="backInOut",Y=`WIX 🤠
Particles!`,k="#ffffff",U=["#ff0000","#00ff00","#0000ff"],F={fontFamily:"Arial",fontSize:64,italic:!1,weight:600,letterSpacing:0,lineHeight:1.2},X=`// This function will be called twice for each particle, because all particles reach the target in two frames.
...
}`,W=()=>Object.assign({},{[b]:{code:`${T}${L}`},DEV_TWO_FRAMES:{code:`${T}${X}`},bezier:{code:`${T}${N}`},pulseColorCycle:{code:`${T}${q}`},timeDistortion:{code:`${T}${H}`},elasticPlop:{code:`${T}${$}`}},...B.map(({name:e,comment:a,definition:o})=>({[e]:{code:`${T}return (particle, animationStartTime, currentTime, canvasDimensions, animationDuration) => {
